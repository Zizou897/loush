...
def post_agence(request):
    ...
    data = {"success": all_is_true, "msg": msg}
...
def post_searcher(request):
    ...
    if (
        ...
        searcher, created = Searcher.objects.get_or_create(
            ...
            email=email,
            phone=phone,
            type_propriete_rechercher=type_propriete_rechercher,
            achat_or_location=achat_or_location,
            nbr_chambre=nbr_chambre,
            nbr_salle_bain=nbr_salle_bain,
            surface_habitable=surface_habitable,
            localisation_souhaite=localisation_souhaite,
            caract_souhaite=caract_souhaite,
            date_demenag_souhaite=date_demenag_souhaite,
            comments_souhaite=comments_souhaite,
        )
        if created:
            msg = "Vous recevrez un mail de la part de Louhsira"
...
def post_owner(request):
    ...
    if verify_email(email):
        ...
        owner, created = Owner.objects.get_or_create(
            ...
            status=status,
            budjet=budjet,
            address_propriete=address_propriete,
            annee_construction=annee_construction,
            surface_habitable=surface_habitable,
            nbr_chambre=nbr_chambre,
            nbr_salle_bain=nbr_salle_bain,
            caracteris_special=caracteris_special,
...
def post_reservation(request):
    ...
    if (
        ...
        reserve, created = Reservation.objects.get_or_create(
            name_of_reserver=name,
            phone=phone,
            email=email,
            propriete_reserve=property_reserve_obj,
        )
        if created:
            reserve.save()
...
def contact_us(request):
    ...
    if not all([name, email, subject]):
        data = {"success": False, "msg": "Veuillez renseigner les champs obligatoires"}
        return JsonResponse(data, safe=False)

    if verify_email(email):
        data = {"success": False, "msg": "Veuillez saisir une adresse email valide"}
        return JsonResponse(data, safe=False)
    ...
    if created:
        ...
        data = {"success": True, "msg": "Message déjà envoyé"}
        return JsonResponse(data, safe=False)
