...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "fichier",
            type=str,
            help="Chemin vers le fichier contenant les types de maisons.",
        )
            ...
            with open(chemin_fichier, "r", encoding="utf-8") as fichier:
                ...
                for ligne in lignes:
                    ...
                    if not existant:
                        ...
                        self.stdout.write(
                            self.style.WARNING(f"⚠️ Déjà présent : {libele}")
                        )

        except FileNotFoundError:
            self.stderr.write(
                self.style.ERROR(f"❌ Fichier non trouvé : {chemin_fichier}")
            )
