...
def home(request):
    ...
    get_property_locations = get_property_location(
        {"publish": True, "status": "à louer"}
    )
...
def catalogue(request):
    ...
    get_all__properties = get_all_properties(
        {
            "publish": True,
        }
    )
    ...
    if request.GET:
        ...
        if prix_min is not None and prix_max is not None:
            if prix_min <= prix_max:
                filters &= Q(prix_propriete__gte=prix_min) & Q(
                    prix_propriete__lte=prix_max
                )
            else:
                sweetify.warning(
                    request,
                    "",
                    text="Le budget minimum doit être inférieur au budget maximum.",
                    persistent="OK",
                )
